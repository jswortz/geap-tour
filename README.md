...
| [Multi-Turn Evaluation](src/eval/demo/multi_turn_eval_demo.ipynb) | Score whole conversations with the `MULTI_TURN_*` rubric metrics — threaded session → `EvalCase(conversation_history)`, no SDK patching. Needs `aiplatform ≥ 1.163`. |
...
| **Deep-dive: [Multi-Turn Evaluation](src/eval/demo/multi_turn_eval_demo.ipynb)** | Score whole conversations with the `MULTI_TURN_*` rubric metrics (task success / general quality / trajectory) — a threaded Agent Engine session → `EvalCase(conversation_history)`, no SDK patching. Requires `aiplatform ≥ 1.163`. |
